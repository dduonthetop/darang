    ...
    with path.open("r", encoding="utf-8-sig", newline="") as f:
        ...
        for row in reader:
            ...
            item = FAQItem(
                ...
                keywords=_split_multi_value(row.get("keywords") or "") or _derive_keywords(question, category),
                manual_files=_split_multi_value(row.get("manual_files") or ""),
                last_editor_id=(row.get("last_editor_id") or "").strip(),
                last_editor_name=(row.get("last_editor_name") or "").strip(),
                last_edited_at=(row.get("last_edited_at") or "").strip(),
            )
            items.append(item)
    return items
